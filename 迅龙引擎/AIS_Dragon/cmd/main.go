...
func main() {
	...
	// 消费主题"Ais"的分区0
	partitionConsumer, err := consumer.ConsumePartition("Ais", 0, sarama.OffsetNewest)
	if err != nil {
		log.Fatalf("Error consuming partition: %v\n", err)
	}
	...
	for msg := range partitionConsumer.Messages() {
		...
		//parseTCPMessage(msg.Value)

		timestamp := msg.Timestamp
		// 将时间戳格式化为你所期望的格式
		formattedTime := timestamp.Format(time.RFC3339)
		fmt.Printf("Message at Offset %d, Key = %s, Value (Hex) = %02x, Received message at time: %s\n ", msg.Offset, string(msg.Key), msg.Value, formattedTime)

		// 将Value中的每个字节以16进制形式打印，以便更好地观察TCP报文的二进制内容
		// for i := 0; i < len(msg.Value); i++ {
		// 	fmt.Printf("%02x ", msg.Value[i])
		// }
		// fmt.Println() // 换行

		// 解析数据包
		parseNetworkPacket(msg.Value)
		fmt.Println("Network packet parsed")

		pacp_info = ""

		pacp_info = fmt.Sprintf("%02x", msg.Value)

		length = uint64(len(msg.Value))
